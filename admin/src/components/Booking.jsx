...
const StatusIndicator = ({status, isEditing, newStatus, onStatusChange}) => {
  return isEditing ? (
    <select value={newStatus} onChange={onStatusChange} className='bg-gray-800/50 text-sm px-2 py-1 rounded focus:outline-none focus:ring-1 focus:ring-orange-500'>
  ...
  <div className={BookingPageStyles.bookingCardHeader}>
    ...
    <div className={BookingPageStyles.bookingExpandIcon}>
      ...
      <span className=' ml-2 text-sm'>
        {isExpanded ? 'Hide Details' : 'Show Detail'}
      </span>
    </div>
  </div>
)
...
const Booking = () => {
  ...
  // Fetch Bookiing Form Server Side
...
const updateStatus = async (id) => {
  try {
    ...
    if(!newStatus){
      window.alter('Please select a status before saving.');
      return;
    }
