...
const deleteLocalFileIfPresent = (filePath) => {
  ...
  fs.unlink(full, (err) => {
    if (err) console.warn('Error deleting file:', full, err);
  });
}
...
export const updateBooking = async (req, res) => {
    ...
    res.json(updated);
        ...
        return res.status(500).json({ success: false, message: err.message || 'Failed to update booking' });
    }
}

// UPDTAE STATUS OF BOOKING ORDER
...
export const deleteBooking = async (req, res) => {
    try {
        ...
        return res.status(500).json({ success: false, message: err.message || 'Failed to delete booking' });
    }
}